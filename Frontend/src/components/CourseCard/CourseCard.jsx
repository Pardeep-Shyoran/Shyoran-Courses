...
const CourseCard = ({ 
  ...
  const handleExportSyllabus = (e) => {
    e.preventDefault()
    e.stopPropagation()
    exportCourseSyllabus(course)
  }

  // --- LIST VIEW MODE ---
  if (viewMode === 'list') {
    return (
      <div className={`${styles.card} ${styles.cardListMode}`}>
        {/* Left Thumbnail */}
        <div className={styles.listThumbnailWrapper}>
          {course.thumbnail ? (
            <img src={course.thumbnail} alt={course.title} className={styles.listThumbnail} />
          ) : (
          ...
          <span className={styles.badge}>
            {totalCount} videos • ⏱️ {durations.totalFormatted}
          </span>
        ...
        <div className={styles.listContent}>
          <h3 className={styles.listTitle} title={course.title}>
            <Link to={`/courses/${enrolledCourseId || course._id}`} style={{ color: 'inherit', textDecoration: 'none' }}>
              {course.title}
            </Link>
          </h3>
          ...
          {course.tags && course.tags.length > 0 && (
            <div className={styles.cardTags} style={{ marginTop: 0 }}>
              {course.tags.slice(0, 4).map((tag, idx) => (
                <span key={idx} className={styles.cardTag}>#{tag}</span>
              ))}
              {course.tags.length > 4 && (
                <span className={styles.cardTag}>+{course.tags.length - 4}</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className={styles.listFooter}>
          {activeMainTab === 'library' && (
            <div className={styles.listProgressWrap}>
          ...
          <div className={styles.actions}>
            ...
            >
              {copiedLink && <span className={styles.copiedToast}>Link Copied! 📋</span>}
        ...
        {course.tags && course.tags.length > 0 && (
          <div className={styles.cardTags}>
            ...
            {course.tags.length > 3 && (
              <span className={styles.cardTag}>+{course.tags.length - 3}</span>
            )}
          </div>
        )}
